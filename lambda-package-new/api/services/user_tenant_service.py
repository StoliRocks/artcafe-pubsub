...
class UserTenantService:
    ...
    async def create_user_tenant_mapping(
        ...
        try:
            ...
            mapping_dict = {
                ...
                "active": 1  # Convert boolean to number for DynamoDB
            }
            ...
            # First create the model, then get its dict representation with our overrides
            model = UserTenant(**fixed_mapping_dict)
            return model
    ...
    async def check_user_access(
        ...
        try:
            mapping = await self.get_user_tenant_mapping(user_id, tenant_id)
            if not mapping or mapping.active == 0:
                return False
