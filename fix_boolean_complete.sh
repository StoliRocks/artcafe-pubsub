...
# Complete fix for boolean values in DynamoDB

echo "Applying complete boolean fix to ArtCafe backend..."

# Fix tenant_service.py - all boolean conversions
cat > tenant_service_fix.py << 'EOF'
import logging
import secrets
import ulid
...
class TenantService:
        ...
        try:
            ...
            if tenant_data.terms_acceptance:
                try:
                    ...
                    await terms_acceptance_service.create_acceptance(
    ...
    async def _initialize_subscriber_tracking(self, tenant_id: str) -> None:
        ...
        try:
            ...
            system_channel_id = f"channel-system-{ulid.new().str.lower()}"
            ...
            system_agent_id = f"agent-system-{ulid.new().str.lower()}"
...
echo "Fix applied. Now uploading to EC2..."