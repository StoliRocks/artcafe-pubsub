...
# Fix the expression values conversion in dynamodb.py
cat <<'EOF' > /tmp/fix_dynamo.py
import re

# Read the file
with open('/opt/artcafe/artcafe-pubsub/api/db/dynamodb.py', 'r') as f:
    content = f.read()

# Find the query_items method and fix it
...
with open('/opt/artcafe/artcafe-pubsub/api/db/dynamodb.py', 'w') as f:
    f.write(content)
...
sudo python3 /tmp/fix_dynamo.py
